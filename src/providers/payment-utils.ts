...
export class PaymentUtils {
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: data => {
            ...
            Promocode.validate(data.code, routeId).then(
              result => {
                if (result['action'] == 'ok') {
                  prompt.data.message = this.translation.instant(
                    'promocode_valid'
                  );
                  Promocode.apply(data.code, 'dumyDeviceId')
                    .then(applyResult => {
  ...
  getProducts(routes: Array<Route>) {
    return this.iap.getProducts(
      routes.map(route => 'com.innapp.dromos.' + route.id.toLocaleLowerCase())
    );
  }

  async buy(productId) {
    return this.iap
      .getProducts(['com.innapp.dromos.' + productId.toLocaleLowerCase()])
      .then(productData => {
  ...
  enableRoute(productId) {
    this.localStorage.setRouteValue(
      productId.substr(productId.lastIndexOf('.') + 1),
      'purchased',
      true
    );
  }
    ...
    if (this.platform.is('cordova')) {
      ...
      return this.iap
        ...
        .then(purchases => {
          ...
          let bundlesFiltered = purchases.filter(
            bundl => bundl.productId.indexOf('bundle') != -1
          );
          let routesFiltered = purchases.filter(
            route => route.productId.indexOf('bundle') == -1
          );

          bundlesFiltered.forEach(bundle => {
            let bundleObject = bundlesLoadAll.filter(dataBundle =>
              bundle.productId
                .toLocaleLowerCase()
                .includes(dataBundle.id.toLocaleLowerCase())
            );

            bundleObject[0].route.forEach(routeId => {
              purchasedBundleRoutes.push(routeId.toLocaleLowerCase());
            });

            this.enableBundle(bundleObject[0]);
          });
          ...
          let purchasesALL = purchases.map(purchase =>
            purchase.productId.substr(purchase.productId.lastIndexOf('.') + 1)
          );
          purchasesALL = purchasesALL.concat(purchasedBundleRoutes);
  ...
  getProductsBundles(bundles: Array<Bundle>) {
    return this.iap.getProducts(
      bundles.map(
        bundle =>
          'com.innapp.dromos.bundle.' +
          (this.platform.is('android')
            ? bundle.id.toLocaleLowerCase()
            : bundle.id)
      )
    );
  }
  ...
  async buyBundle(bundle) {
    return this.iap
      .getProducts([
        'com.innapp.dromos.bundle.' + (this.platform.is('android')
          ? bundle.id.toLocaleLowerCase()
          : bundle.id)
      ])
      .then(productData => {
  ...
  //   Bundle.validate(bundle, routeId).then(
  //     result => {
  //       if (result['action'] == 'ok') {
  //         Bundle.apply(bundle, 'dumyInfoDeviceId')
  //           .then(applyResult => {
  //             if (applyResult['action'] == 'ok') {
  //               setTimeout(() => {
  //                 okCallback();
